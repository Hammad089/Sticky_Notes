...
const WednesdayNotes = () => {
    ...
    const loadNotes = async () => {
        try {
            const savedNotes = await AsyncStorage.getItem('notes_wednesday');
            if (savedNotes !== null) {
    ...
    const handleEditNote = (note) => {
        navigation.navigate('CreateNote', { note, noteKey: 'notes_wednesday', updateNotes: loadNotes });
    };
    ...
    const saveNotes = async (updatedNotes) => {
        try {
            await AsyncStorage.setItem('notes_wednesday', JSON.stringify(updatedNotes));
            loadNotes();
        } catch (error) {
        ...
        <View style={styles.container}>
            ...
            >
                {notes.length === 0 ? (
                    ...
                    <Text style={styles.emptyText}>{selectedLanguage.create_your_first_note}</Text>
                    </View>
                ) : (
                    renderNotes()
                )}
            ...
            <TouchableOpacity
                ...
                onPress={() => navigation.navigate('CreateNote', { noteKey: 'notes_wednesday', updateNotes: loadNotes })}
            >
